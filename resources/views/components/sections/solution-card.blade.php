...
<div class="mb-20 overflow-hidden rounded-xl shadow-lg bg-white" data-aos="fade-up" data-aos-delay="{{ $delay }}">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-0">
        <!-- Image -->
        <div class="relative h-80 lg:h-auto overflow-hidden bg-gray-200 {{ $imagePosition === 'left' ? 'order-2 lg:order-1' : '' }}">
            <img src="{{ asset($image) }}" alt="{{ $title }}"
                class="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
        </div>

        <!-- Content -->
        <div class="p-8 sm:p-12 lg:p-16 flex flex-col justify-center {{ $imagePosition === 'left' ? 'order-1 lg:order-2' : '' }}">
