...
@php
    $heroImage = filled($contact?->getFirstMediaUrl('contact-hero'))
        ? $contact->getFirstMediaUrl('contact-hero')
        : asset('images/HOME/SECTION 5/inside news/NEWS 3/1.jpg');
@endphp
...
<section class="relative w-full min-h-[70vh] sm:min-h-[80vh] lg:min-h-screen flex items-center justify-center overflow-hidden">
    ...
    <div class="relative z-10 text-white px-6 sm:px-10 max-w-4xl text-center" data-aos="fade-up">
        <h1 class="text-5xl sm:text-6xl lg:text-7xl font-light mb-6">{{ $contact?->hero_title ?? 'Get In Touch' }}</h1>
        <p class="text-xl sm:text-2xl text-gray-100">{{ $contact?->hero_subtitle ?? "We'd love to hear from you. Let's create something extraordinary together." }}</p>
    </div>
</section>
