@php
    $galleryImages = [
        ...
        [
            'src' => 'https://hommes.studio/wp-content/uploads/Get-To-Know-12-Of-The-Most-Popular-Interior-Design-Styles_2.jpg',
            'alt' => 'Interior style showcase',
            'span' => '',
        ],
        [
...
{{--        </div>--}}
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 auto-rows-[190px] sm:auto-rows-[210px] lg:auto-rows-[180px] gap-4 sm:gap-5">
            @foreach ($galleryImages as $index => $image)
                <figure class="group relative overflow-hidden rounded-2xl border border-black/10 shadow-[0_12px_28px_rgba(20,20,20,0.1)] {{ $image['span'] }}"
                    data-aos="fade-up" data-aos-delay="{{ 60 + ($index * 40) }}">
                    <img src="{{ $image['src'] }}" alt="{{ $image['alt'] }}" loading="lazy"
                        class="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
                    <div class="absolute inset-0 bg-gradient-to-t from-black/35 via-black/5 to-transparent opacity-70 group-hover:opacity-85 transition-opacity duration-500"></div>
                </figure>
            @endforeach
        </div>
    </div>
